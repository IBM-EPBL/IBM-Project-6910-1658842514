...
conn = ibm_db.connect(
    "DATABASE=bludb;HOSTNAME=2d46b6b4-cbf6-40eb-bbce-6251e6ba0300.bs2io90l08kqb1od8lcg.databases.appdomain.cloud;PORT=32328;SECURITY=SSL;SSLservercertificate=DigiCertGlobalRootCA.crt;UID=gth80312;PWD=fEQO2rLE1hFMbDyJ",
    '',
    '')
...
def sendemail(toemail, subject, content):
    message = Mail(
        from_email='pexpensetracker@gmail.com',
        to_emails=toemail,
        subject=subject,
        html_content=content)
    try:
        sg = SendGridAPIClient(
            os.environ.get('Sendgrid_Apikey'))
...
def Register():
    if "user_id" not in session:
        if request.method == 'POST':
            ...
            if account:
                flash(
                    'Account with email id already exists, Please login or use another email')
                ...
                content = "Dear " + \
                    str(name) + ''' ,<br><br>Thank you for registering in the application.<br>
                ...
                if request.method == "GET":
                    ...
                    if account:
                        ...
                        return render_template(
                            'profile.html', account=account, budget=budget, month=month)
                    else:
                        flash('Error', error)
                        return redirect(url_for("login"))
...
def dashboard():
    ...
    if "user_id" in session:
        ...
        for transaction in transactionslist:
            total[transaction['CATEGORY_ID'] - 1] += transaction['AMOUNT']
        ...
        df = pd.DataFrame({'Categories': [
                          'Rent and EMI', 'Savings', 'Groceries', 'others', 'Not Set'], 'Total': total})
        df = df[df['Total'] >= 1]
        pie_chart = px.pie(df,
                           values='Total',
                           names='Categories',
                    ...
                    if session['remaining'] <= int(amount):
                        ...
                        content = '''Dear ''' + str(
                            session['name']) + ''', <br><br>
                                  ...
                                  <div style="text-align: center;"> You have exceeded your Monthly limit. The amount of Rs. ''' + str(
                            float(amount) - session['remaining']) + ''' is exceeded from the actual amount.
                        ...
                        sendemail(toemail, subject, content)
                        flash('Email sent to the registered email ID')
    ...
    if "user_id" in session:
        ...
        if request.method == 'POST' and request.form['submit'] == 'modifyexpense':
            ...
            if session['remaining']:
                if session['remaining'] >= 0:
                    if session['remaining'] <= (
                            int(float(amount)) - int(transaction['AMOUNT'])):
                        toemail = email
                        toemail = email
                        subject = "Alert !"
                        content = '''Dear ''' + str(session['name']) + ''', <br><br>

                                  <div style="text-align: center;"> You have exceeded your Monthly limit. The amount of Rs. ''' + \
                            str((int(float(amount)) - int(transaction['AMOUNT'])) - session['remaining']) + ''' is exceeded from the actual amount.
                                   Kindly, maintain the expense as per the registered actual limit, or change your actual Limit for monthly expenses.<br><br>
        ...
        if request.form['submit'] == 'email':
            ...
            if account:
                ...
                content = "Hey " + str(session['name']) + ''' ,  <br><br>
                                ...
                                Personal expense Tracker'''
                sendemail(toemail, subject, content)
                flash('Email with OTP sent to given Email ID')
